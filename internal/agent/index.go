...
func nextFrame(r *bufio.Reader) (FrameMeta, []byte, error) {
    ...
    if err != nil { return FrameMeta{}, nil, err }
    var fm FrameMeta
...
func preadSection(f *os.File, off int64, length int64) ([]byte, error) {
    if f == nil { return nil, errors.New("nil file") }
...
func oldestIndex(dir string) (string, error) {
    ...
    for _, e := range ents {
        ...
        if len(name) == len("2006-01-02") && strings.Count(name, "-") == 2 {
            hasDay = true
            if name < earliestDay { earliestDay = name }
        }
    }
    if hasDay {
        ...
        for _, de := range dayEnts {
            n := de.Name()
            if strings.HasSuffix(n, ".wal.idx") && n < oldest { oldest = n }
        }
        if oldest == "~" { return "", fmt.Errorf("no index files in %s", dayDir) }
    ...
    if oldest == "~" { return "", fmt.Errorf("no index files in %s", dir) }
...
func nextIndexAfter(curIdxPath string) (string, bool, error) {
    ...
    for _, e := range ents {
        ...
        name := e.Name()
        if len(name) == len("2006-01-02") && strings.Count(name, "-") == 2 {
            if name > curDay && (nextDay == "" || name < nextDay) { nextDay = name }
        }
    }
    if nextDay == "" { return "", false, nil }
