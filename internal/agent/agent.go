...
func Run(ctx context.Context, cfg Config) error {
    ...
    if err := os.MkdirAll(cfg.StateDir, 0o700); err != nil { return fmt.Errorf("state dir: %w", err) }
    ...
    if err != nil { return fmt.Errorf("open idx: %w", err) }
    defer idx.Close()
    if st.IdxOffset > 0 { if _, err := idx.Seek(st.IdxOffset, io.SeekStart); err == nil { r.Reset(idx) } }

    // Open current gz if known
    var gz *os.File
    if st.CurGz != "" {
        if f, err := openGz(filepath.Join(filepath.Dir(st.IdxPath), st.CurGz)); err == nil { gz = f }
    }
    ...
    for {
        ...
        if nerr != nil {
            if errors.Is(nerr, os.ErrClosed) { return nerr }
        ...
        if gz == nil || filepath.Base(st.CurGz) != fm.File {
            ...
            if gerr != nil { time.Sleep(cfg.PollInterval); continue }
            gz = ngz
            st.CurGz = fm.File
        }
        if cfg.Meta {
        ...
        if rerr != nil { time.Sleep(cfg.PollInterval); continue }
        if cfg.Verify { _ = verifyFrame(fm, io.NopCloser(bytes.NewReader(b))) }
...
func trySend(cfg Config, httpClient *http.Client, batch *[]batchFrame, batchBytes *int, st *state, curIdxBase string, gz **os.File, lastSend time.Time, back *backoff) {
    ...
    if !hard && !resourcesOK(cfg) { return }
    ...
    if err != nil { back.Sleep(); return }
    defer resp.Body.Close()
    if resp.StatusCode/100 != 2 { back.Sleep(); return }
